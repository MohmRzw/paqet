...
- آیا می‌خواهید پورت‌ها را به صورت **دسته‌جمعی (Bulk)** وارد کنید یا نه
- اگر `forward` بزنید، برای هر Rule می‌پرسد:
  - `Local listen` (مثل `127.0.0.1:7001`)
  - `Target` (مثل `1.2.3.4:443` یا `your-real-domain.com:443`)
  - `Protocol` (`tcp` یا `udp`)

اگر حالت Bulk را بزنید، می‌توانید چند پورت را یکجا با `,` وارد کنید:

- حالت ساده (لوکال و تارگت یکسان): `7001,7002,7003`
- حالت مپ کردن لوکال به تارگت: `7001:443,7002:8443,7003:9443`

در Bulk فقط یک بار Host و Protocol را می‌دهید و بقیه Ruleها خودکار ساخته می‌شود.
